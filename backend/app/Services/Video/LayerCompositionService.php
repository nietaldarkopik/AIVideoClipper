...
class LayerCompositionService
{
        ...
        foreach ($layers as $i => $layer) {
            ...
            switch ($type) {
                ...
                case 'logo':
                    ...
                    [$lines, $videoLabel] = $this->buildImageLayer($layer, $videoLabel, $targetWidth, $duration, $inputIndex, $suffix);
                    $graph = array_merge($graph, $lines);
                    $inputArgs[] = '-i';
                    $inputArgs[] = $resolvePath($imagePath);
                    $inputCount++;
                    break;
     ...
     */
    private function buildImageLayer(array $layer, string $videoLabel, int $targetWidth, float $duration, int $inputIndex, string $suffix): array
    {
